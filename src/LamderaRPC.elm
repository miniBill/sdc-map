    ...
    case Json.decodeValue argsDecoder rpcArgsJson of
        Ok rpcArgs ->
            let
                ...
                resolveRpc : ( String, Json.Value ) -> Cmd msg
                resolveRpc value =
                    rpcOut
                        (Json.object
                            [ ( "t", Json.string "qr" )
...
asTaskString requestBody endpoint =
    Http.task
        ...
        , resolver = Http.stringResolver <| customResolver (\_ text -> Ok text)
        , timeout = Just 15000
        }
...
handleEndpointString fn args model =
    case args.body of
        Raw string ->
            let
                ( response, newModel, newCmds ) =
                    fn args.sessionId model string
            in
            case response of
                Ok value ->
